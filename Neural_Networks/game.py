...
class AIGame:
    ...
    def init_game_objects(self):
        ...
        # COINS
        # -----
        # IMPROVED: Coins now spawn in the middle vertical zone to be more reachable
        ...
        for index in range(self.window_width // self.coin_spacing):
            _coin = Coin(
                ...
                y_position=random.randint(
                    middle_spawn_lower, middle_spawn_upper
                ),
            )
        ...
        for _ in range(length):
            ...
            for index, coin in enumerate(self.coins):
                ...
                if (coin.x_position + coin.width // 2) <= 0:
                    # IMPROVED: Reset coin to middle zone for consistency
                    middle_spawn_lower = self.window_height // 3
                    middle_spawn_upper = (2 * self.window_height) // 3
                    coin.reset(
                        y_position=random.randint(
